"""
patch_code_stop.py — Patch Code.class:
  Code.gameAF()V  →  ChatRouter.stopCurrentAuto()V

Thay Methodref Code.gameAF()V trong method gameAF()V cua Code.class
bang ChatRouter.stopCurrentAuto()V

Dung de: khi user bam nut Tat Auto trong menu, AutoLevel cung bi tat.
"""
import struct, sys

def patch_code_stop(class_file):
    with open(class_file, 'rb') as f:
        data = bytearray(f.read())

    if b"stopCurrentAuto" in data:
        print("⚡ Code.class đã có stopCurrentAuto, bỏ qua!")
        return
    ...
    idx_utf8_name = cp_count + 2    # UTF8 "stopCurrentAuto"
    idx_utf8_desc = cp_count + 3    # UTF8 "()V"
    idx_nat = cp_count + 4          # NameAndType stopCurrentAuto:()V
    idx_mr = cp_count + 5           # Methodref ChatRouter.stopCurrentAuto
    ...
    # 3. UTF8 "stopCurrentAuto"
    method_bytes = b"stopCurrentAuto"
    new_cp_bytes.append(1)
    ...
    for mr_idx, c_idx, nat_idx in mr_list:
        ...
        if c_name == 'Code':
            ...
            if name == 'gameAF' and desc == '()V':
                target_mr_idx = mr_idx
                print(f"Found Code.gameAF()V at CP#{mr_idx}")
                break

    if target_mr_idx == -1:
        print("❌ Khong tim thay Methodref Code.gameAF()V")
        return
    ...
    # === Scan: thay invokestatic Code.gameAF()V → ChatRouter.stopCurrentAuto()V ===
    # CHI patch trong GameScr.class (caller) KHONG patch trong Code.class
    # Vi Code.gameAF() goc van can chay — chi them hook truoc no
    ...
    if patched_count == 0:
        print("⚠️ Khong tim thay invokestatic Code.gameAF()V trong bytecode!")
    else:
        print(f"✅ Patched {patched_count} call(s): Code.gameAF()V → ChatRouter.stopCurrentAuto()V")
